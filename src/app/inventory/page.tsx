        ...
        <table className="w-full text-sm table-auto" style={{ tableLayout: 'fixed' }}>
          ...
          <tbody>
            <tr>
              ...
              <td className="px-3 pb-2 align-bottom">
                <label className="block text-xs mb-1">Base unit <Tip text="The unit you use in recipes (e.g., g, ml, each)." /></label>
                <select className="border border-neutral-700 bg-black p-2 rounded w-full" value={form.base_unit} onChange={e => setForm({ ...form, base_unit: e.target.value })}>
                  <option>g</option><option>ml</option><option>each</option><option>oz</option><option>lb</option>
                </select>
              </td>
              <td className="px-3 pb-2 align-bottom">
                <label className="block text-xs mb-1">Purchase <Tip text="What you buy from vendors (e.g., kg, L, case)." /></label>
                <select className="border border-neutral-700 bg-black p-2 rounded w-full" value={form.purchase_unit} onChange={e => setForm({ ...form, purchase_unit: e.target.value })}>
                  <option>kg</option><option>l</option><option>case</option><option>each</option><option>lb</option>
                </select>
              </td>
              <td className="px-3 pb-2 align-bottom">
                <label className="block text-xs mb-1">Pack → Base <Tip text="How many base units are in ONE purchase unit (e.g., 1 kg = 1000 g)." /></label>
                <input className="border border-neutral-700 bg-black p-2 rounded w-full" type="number" step="0.0001"
                  value={form.pack_to_base_factor}
                  onChange={e => setForm({ ...form, pack_to_base_factor: Number(e.target.value) })}/>
              </td>
              <td className="px-3 pb-2 align-bottom">
                <label className="block text-xs mb-1">Last price ($) <Tip text="Price for ONE purchase unit last time you bought it." /></label>
                <input className="border border-neutral-700 bg-black p-2 rounded w-full" type="number" step="0.01"
                  value={form.last_price} onChange={e => setForm({ ...form, last_price: Number(e.target.value) })}/>
              </td>
              <td className="px-3 pb-2 align-bottom">
                <button onClick={addItem} className="w-28 bg-white text-black font-medium rounded px-4 py-2">Add item</button>
              </td>
            </tr>
          </tbody>
        </table>
