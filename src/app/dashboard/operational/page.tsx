...
export default async function OperationalDashboard(props: any) {
  ...
  const hasPurchaseChartData = weekdayPurchase.some((x) => Number(x.value) > 0);
    ...
    <div>
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-3 mb-6">
        ...
        <KpiCard
          ...
          formula="SUM(inventory_receipts.qty_base)"
        />
        <KpiCard label="Focus" value={"Inventory & Freshness"} hint="Operational dashboard focuses on stock, purchases, and waste prevention." />
      </div>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-4">
        <div className="rounded border border-neutral-800 p-4">
          ...
          {/* Empty-state polish */}
          {hasPurchaseChartData ? (
            <WeekdayBars data={weekdayPurchase} />
          ) : (
            <div className="text-sm opacity-70 rounded border border-neutral-800 bg-neutral-950/40 p-3">
              No purchasing recorded in this range.
            </div>
          )}
        </div>
