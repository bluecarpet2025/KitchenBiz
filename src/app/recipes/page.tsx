...
export default async function RecipesPage() {
    ...
    return (
      <main className="max-w-6xl mx-auto p-6">
        <div className="flex items-center justify-between">
          <h1 className="text-2xl font-semibold">Recipes</h1>
          <Link href="/recipes/new" className="px-3 py-2 border rounded-md text-sm hover:bg-muted">New Recipe</Link>
        </div>
  ...
  for (const r of recipes) {
    ...
    if (lines.length === 0) { byRecipe.set(r.id, {min: 0}); continue; }
          ...
          <tbody>
            {recipes.map(r => (
              <tr key={r.id} className="border-t">
                ...
                <td className="p-2 text-right tabular-nums">{byRecipe.get(r.id)?.min ?? 0}</td>
