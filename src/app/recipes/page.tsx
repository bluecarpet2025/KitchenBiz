...
export default async function RecipesPage() {
  ...
  const { data: recipeIngs } = recipes?.length
    ? await supabase
  ...
  const onMap = new Map((onhand ?? []).map((r: any) => [r.item_id, Number(r.qty_on_hand || 0)]));
  const totalOnHand = Array.from(onMap.values()).reduce((a,b)=>a+b,0);

  const byRecipe = new Map<string, number>();
  for (const r of recipes ?? []) {
    const lines = (recipeIngs ?? []).filter((x) => x.recipe_id === r.id);
    let minMakeable = Infinity;
    if (lines.length === 0) minMakeable = 0;
    for (const ln of lines) {
      const have = onMap.get(ln.inventory_item_id) ?? 0;
      const need = Number(ln.qty_per_recipe) || 0;
      if (need <= 0) continue;
      minMakeable = Math.min(minMakeable, Math.floor(have / need));
    }
    if (!Number.isFinite(minMakeable)) minMakeable = 0;
    byRecipe.set(r.id, Math.max(0, minMakeable));
  }
    ...
    <main className="max-w-6xl mx-auto p-6">
      ...
      {totalOnHand === 0 && (
        <p className="mt-3 text-xs text-amber-400">
          Heads up: Makeable is 0 because there’s no stock on hand yet. Record a{" "}
          <Link href="/inventory/purchase" className="underline">Purchase</Link> or commit a{" "}
          <Link href="/inventory/counts/new" className="underline">Count</Link>.
        </p>
      )}
          ...
          <tbody>
            {(recipes ?? []).map((r) => (
              <tr key={r.id} className="border-t">
                ...
                <td className="p-2 text-right tabular-nums">{byRecipe.get(r.id) ?? 0}</td>
            ...
            {(recipes ?? []).length === 0 && (
              <tr><td colSpan={4} className="p-2 text-sm text-muted-foreground">No recipes yet.</td></tr>
            )}
          </tbody>
        </table>
      </div>
