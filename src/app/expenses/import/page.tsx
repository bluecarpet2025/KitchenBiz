...
function parseCSV(text: string): Row[] {
  ...
  for (const h of REQUIRED_HEADERS)
    if (idx(h) === -1) throw new Error(`Missing header: ${h}`);
...
export default function ImportExpenses() {
  ...
  // --- PLAN GATE (client-safe via API route) -----------------
  useEffect(() => {
    (async () => {
      try {
        ...
        const canUseFeature = (plan: any, key: string) => plan?.features?.includes?.(key);
        setAllowed(canUseFeature(plan, "expenses_access"));
      } catch {
        setAllowed(false);
      }
    })();
  ...
  useEffect(() => {
    (async () => {
      ...
      const { data: prof, error } = await supabase
        ...
        .eq('id', uid)
        .maybeSingle();
      if (error || !prof?.tenant_id)
        return setErr(error?.message || 'No tenant. Visit /app to initialize.');
  ...
  if (allowed === false) {
    return (
      <main className="max-w-3xl mx-auto p-6 text-center">
        <h1 className="text-2xl font-semibold mb-2">Expenses Import</h1>
        <p className="opacity-80">Your current plan doesn’t include Expenses features.</p>
        <Link
          href="/profile"
          className="inline-block mt-3 border rounded px-4 py-2 hover:bg-neutral-900"
        >
          Upgrade plan
        </Link>
      </main>
  ...
  async function importNow() {
    ...
    const bad = rows.find(
      (r) => !r.name || !r.base_unit || !r.purchase_unit || !r.pack_to_base_factor
    );
    if (bad) {
      setBusy(false);
      return setErr('Some rows are missing required fields.');
    }
    ...
    const { error } = await supabase.from('expenses_items').insert(payload);
    setBusy(false);
    if (error) setErr(error.message);
    else {
      setOk(`Imported ${rows.length} items ✅`);
      setRows([]);
    }
  }
    ...
    <div className="space-y-4">
      <div className="flex items-center justify-between">
        <div className="flex items-baseline gap-4">
          ...
          <a className="underline text-sm" href="/expenses">
            ← Back
          </a>
        </div>
        <div className="flex gap-2">
          <button
            ...
            className="text-sm border rounded px-3 py-2 hover:bg-neutral-900"
          >
            Download template
          </button>
          <button
      ...
      {rows.length > 0 && (
        <>
          <div className="text-sm">
            {rows.length} rows parsed. Preview (up to 20):
          </div>
            ...
            <tbody>
              {rows.slice(0, 20).map((r, i) => (
                <tr key={i} className="bg-neutral-950/60 rounded">
                  ...
                  <td className="px-3 py-2 rounded-r">
                    ${Number(r.last_price || 0).toFixed(2)}
                  </td>
                </tr>
              ))}
            </tbody>
          ...
          <button
            disabled={busy}
            onClick={importNow}
            className="bg-white text-black font-medium rounded px-4 py-2"
          >
